  ...
  <table class="table">
    ...
    <tbody>
      <form method="post" enctype="multipart/form-data">
        ...
        <tr>
          <td>
            <a href="./index.php?admin=view_sanpham&IDSanPham=<?php echo $row['IDSanPham'];?>" style="text-decoration: none; color: black;"><p name="tensanpham"><?php echo $row['TenSanPham'];?></p></a>
          </td>
          ...
          <td>  
            <a href="#" class="btn btn-outline-warning mb-3 btn-lg" style="text-decoration: none; font-weight: bold;" onclick="hienThiDialog()">Thay đổi số lượng</a>
            <a href="./index.php?admin=deletegiohang&IDSanPham=<?php echo $row['IDSanPham'];?>&IDTaiKhoan=<?php echo $idtaikhoan;?>" class=" text-danger btn btn-outline-danger  mb-3 btn-lg"style="text-decoration: none; font-weight: bold;">Xóa</a>
            <div id="dialog" style="display: none;">  
              <form method="POST">
                ...
                <a href="" onclick="thayDoiSoLuong()" style="text-decoration: none; color: black;"><button type="submit"class="btn btn-outline-success  mb-1 btn-lg" name="form_click_capnhap">OK</button></a>
              </form>  
            </div>
            <script>
        ...
        <div class="clearfix" style=" float: right;">
          ...
          <p style="height: 150px;">
            <a href="./index.php?admin=pay&IDTaiKhoan=<?php echo $idtaikhoan;?> " class="text-center btn btn-outline-success" id="btn-tooltip" name="form_click_ThanhToan" style="text-decoration: none; margin-top:-5px; font-weight: bold;">Thanh Toán</a>
          </p>
          </div>
          <!-- Below are suggestions for the button -->
          <div id="tooltip"><p class="text-center">Khi nhấn thanh toán sẽ không thể hủy được đơn hàng chỉ hủy đơn trực tiếp lúc shipper giao hàng</p></div>
          <script>
...
<?php
          if (isset($_POST['form_click_capnhap'])) {
            ...
            if ($kq_CapNhap === TRUE) {
              ?>
              <script>
                  alert("Bạn đã cập nhập bài viết sản phẩm <?php echo 'MaSP:'.$idSanPham.' MaTaiKhoan: '.$idtaikhoan.' Số lượng: '.$soLuong; ?>");
